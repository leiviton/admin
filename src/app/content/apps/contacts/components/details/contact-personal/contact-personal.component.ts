...
export class ContactPersonalComponent implements OnInit {
    ...
    ngOnInit() {
        this.contact = this._contacts.viewing;

        this._contacts
            .getSimilarProfiles()
            .subscribe(profiles => (this.similarProfiles = profiles));
    }

    getAvatar(contact: Contact): Avatar {
        return {
            name: contact.name,
